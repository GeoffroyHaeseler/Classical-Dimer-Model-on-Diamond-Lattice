...
extern double g_NSP[2 * xdiv][2 * xdiv];
...
void nspInitialisation() {
	...
	for (q0 = 0; q0 < 2 * xdiv; q0++) {//We go through the whole reciprocal lattice
		for (q1 = 0; q1 < 2 * xdiv; q1++) {
			Q0 = (double(q0) - 1 * xdiv) * xmult / (double(xdiv));
			Q1 = sqrt(2) * (double(q1) - 1 * ydiv) * ymult / (double(ydiv));
			
			//(h, h, 0)  (0, 0, l)
			g_Q[q0][q1][0] = Q0 * (RecL[0][0] + RecL[1][0] + 2 * RecL[2][2]) + Q1 * (RecL[0][0] + RecL[1][0]);
			g_Q[q0][q1][1] = Q0 * (RecL[0][1] + RecL[1][1] + 2 * RecL[2][2]) + Q1 * (RecL[0][1] + RecL[1][1]);
			g_Q[q0][q1][2] = Q0 * (RecL[0][2] + RecL[1][2] + 2 * RecL[2][2]) + Q1 * (RecL[0][2] + RecL[1][2]);
			...
			double n[3] = { -1 / sqrt(2), 1 / sqrt(2), 0 }; // normal vector to the plan (h,h,0) (0,0,l)
			for (ispin = 0; ispin < 4; ispin++) {
				...
				ScalarProduct = dotProduct(spinParallele, g_QHat[q0][q1]);
			...
			for (i2 = 0; i2 < L; i2++) {
				for (itri = 0; itri < 4; itri++) {
					for (ispin = 0; ispin < 4; ispin++) {
						...
						if (itri == 1) {
							g_SpinPosition[i0][i1][i2][itri][ispin][0] += ReaL[0][0];
							g_SpinPosition[i0][i1][i2][itri][ispin][1] += ReaL[0][1];
							g_SpinPosition[i0][i1][i2][itri][ispin][2] += ReaL[0][2];
						}
						if (itri == 2) {
							g_SpinPosition[i0][i1][i2][itri][ispin][0] += ReaL[1][0];
							g_SpinPosition[i0][i1][i2][itri][ispin][1] += ReaL[1][1];
							g_SpinPosition[i0][i1][i2][itri][ispin][2] += ReaL[1][2];
						}
						if (itri == 3) {
							g_SpinPosition[i0][i1][i2][itri][ispin][0] += ReaL[2][0];
							g_SpinPosition[i0][i1][i2][itri][ispin][1] += ReaL[2][1];
							g_SpinPosition[i0][i1][i2][itri][ispin][2] += ReaL[2][2];
						}

						if (ispin == 1) {
							g_SpinPosition[i0][i1][i2][itri][ispin][0] -= 0.5 * ReaL[0][0];
							g_SpinPosition[i0][i1][i2][itri][ispin][1] -= 0.5 * ReaL[0][1];
							g_SpinPosition[i0][i1][i2][itri][ispin][2] -= 0.5 * ReaL[0][2];
						}
						if (ispin == 2) {
							g_SpinPosition[i0][i1][i2][itri][ispin][0] -= 0.5 * ReaL[1][0];
							g_SpinPosition[i0][i1][i2][itri][ispin][1] -= 0.5 * ReaL[1][1];
							g_SpinPosition[i0][i1][i2][itri][ispin][2] -= 0.5 * ReaL[1][2];
						}
						if (ispin == 3) {
							g_SpinPosition[i0][i1][i2][itri][ispin][0] -= 0.5 * ReaL[2][0];
							g_SpinPosition[i0][i1][i2][itri][ispin][1] -= 0.5 * ReaL[2][1];
							g_SpinPosition[i0][i1][i2][itri][ispin][2] -= 0.5 * ReaL[2][2];
						}
	...
	for (q0 = 0; q0 < 2 * xdiv; q0++) {//loop over the reciprocal lattice
		for (q1 = 0; q1 < 2 * xdiv; q1++) {
			...
			if (sqrt(normsq) < g_qlim) {
						...
						for (i2 = 0; i2 < L; i2++) {
							for (itri = 0; itri < 4; itri++) {
								for (ispin = 0; ispin < 4; ispin++) {
									...
									//Only spinUsefull is taken into account
				...
				}//end f the loop on the real lattice
			}
			...
			g_NSP[q0][q1] += std::abs(norm(unpolarised));
...
void endNSPMeasures() {
	...
	for (q0 = 0; q0 < 2 * xdiv; q0++) {
		for (q1 = 0; q1 < 2 * xdiv; q1++) {
			...
			g_NSP[q0][q1] /= double(g_Nspins) * g_nspCount;
			g_BraggsPeak[q0][q1] /= double(g_Nspins) * g_nspCount;
			myfile << X << " " << Y << " " << g_NSP[q0][q1] << " " << g_BraggsPeak[q0][q1] << std::endl;
