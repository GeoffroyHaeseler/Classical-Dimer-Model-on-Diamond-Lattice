...
void addAWorm() {
	...
	int dimer = g_lattice[x][y][z][rng];
	if (rng2 < g_proba[0][dimer]) {
		g_lattice[x][y][z][rng] = 0;
		wormSize += 1;
	}

	else if (g_proba[0][dimer] <= rng2 && rng2 < g_proba[0][dimer] + g_proba[1][dimer]) {
		g_lattice[x][y][z][rng] = 1;
		wormSize += 1;
	}

	else if (g_proba[0][dimer] + g_proba[1][dimer] <= rng2 && rng2 < g_proba[0][dimer] + g_proba[1][dimer] + g_proba[2][dimer]) {
		g_lattice[x][y][z][rng] = 2;
		wormSize += 1;
	}

	else {
		g_lattice[x][y][z][rng] = 3;
		wormSize += 1;
	}
