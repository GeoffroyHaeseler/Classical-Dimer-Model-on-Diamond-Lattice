...
void growTheWorm(int* x, int* y, int* z, char* tetra, int* n) {
	...
	for (i = 0; i < 4; i++) {//We look the four spins arround us. Each one is coming from a different up tetrahedron.
	...
	//We choose spin 0
	if (0 <= rng && rng < g_proba[0][*n]) { next(*x, *y, *z, *tetra, *n, 0, 1, x, y, z, tetra, n); }
	...
	//We choose spin 3
	else if (g_proba[0][*n] + g_proba[1][*n] + g_proba[2][*n] <= rng && rng <= 1) { next(*x, *y, *z, *tetra, *n, 3, 1, x, y, z, tetra, n); }
